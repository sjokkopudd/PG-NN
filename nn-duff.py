# -*- coding: utf-8 -*-
"""
Created on Mon May  3 15:05:51 2021
...
df_train = pd.read_csv("C:/Users/laach/Documents/Skole/Masteroppgave/Master2/Code2/train-duff-redo.csv")
df_train = df_train.drop(columns=['Unnamed: 0'],axis=1)
df_test = pd.read_csv("C:/Users/laach/Documents/Skole/Masteroppgave/Master2/Code2/test-duff-redo.csv")
df_test = df_test.drop(columns=['Unnamed: 0'],axis=1)

x_train = df_train.iloc[:,0:15]
y_train = df_train.iloc[:,15:16]
x_test = df_test.iloc[:,0:15]
y_test = df_test.iloc[:,15:16]
...
test_plots = np.vstack((test_plot_x,test_plot_y)).T

x_train2_pre = x_train[['4','9']] #[x_dot,x]
x_train2 = []

for i2 in range(0,(len(x_train2_pre))):
    
    dt = (x_train.iloc[i2,14]-x_train.iloc[i2,13])
    #x_train2.append(dt*(x_train.iloc[i2,9]**3)) #x^3
    x_train2.append(dt*(math.cos(0.2*x_train.iloc[i2,14]))) #cos
    #x_train2.append([dt*(math.cos(0.2*x_train.iloc[i2,14])),dt*(x_train.iloc[i2,9]**3)]) #cos x^3
...
zeros_noise = np.zeros([row,5])
noise = np.random.normal(mu, sigma, [row,col-5])
noise = np.concatenate((noise,zeros_noise),axis=1) 
...
for normal_nn in range(0,nn_number): 
    ...
    for i in range(0,len(x_test)):
        ...
        pred = model.predict(np.array([predictions[i],])) #x_dotdot
        dt = x_test.iloc[i,14] - x_test.iloc[i,13]
        new_input = predictions[i][1:5]
        new_input.append(new_input[-1] + dt*pred[0][0]) #x_dot + x_dotdot
        new_input.extend(predictions[i][6:10]) #x
        new_input.append(new_input[-1] + dt*new_input[4]) #x+x_dot
    ...
    pred_plots = np.vstack((pred_plot_x,pred_plot_y)).T
...
for input_layer in range(0,4):
    ...
    for guided_nn in range(0,nn_number):
        ...
        if input_layer == 1:
            x12 = Input(shape=(input_shape,))
            x22 = Input(shape=(1,))
            x2 = Dense(16,activation='relu')(x12)     
...
# =============================================================================
#             if i != (len(x_test)-1) :
#                 dt = (x_test.iloc[i+1,14]-x_test.iloc[i,14])
#             else: 
#                 dt = (x_test.iloc[i,14]-x_test.iloc[i,13])
# =============================================================================
            dt = x_test.iloc[i,14] - x_test.iloc[i,13]
            #side_input = dt*(predictions[i][9]**3)
            side_input = dt*(2.3*math.cos(0.2*x_test.iloc[i,14]))
            #side_input = [dt*(2.3*math.cos(0.2*x_test.iloc[i,14])),dt*(predictions[i][9]**3)]
...
guided_nn_results[3][13] = guided_nn_results[3][12]
#normal_nn_results[49] = normal_nn_results[48]
...
t = np.round(np.linspace(0, 25, len(df_nn_x)),2)
...
#bar_plot.savefig('bar_plot-duff-x3.pdf')
#line_plot_nn_x.savefig('nn_x-duff.pdf')
line_plot_nn_y.savefig('nn_duff-y-noise02.pdf')
#line_plot_l1_x.savefig('l1_x-duff.pdf')
line_plot_l1_y.savefig('l1_duff-y-noise02.pdf')

# =============================================================================
# loss_plot = plt.figure()
# plt.plot(histories[0][1].history['loss'])
# plt.plot(histories[0][1].history['val_loss'])
