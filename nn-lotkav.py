...
df_train = pd.read_csv("C:/Users/laach/Documents/Skole/Masteroppgave/Master2/Code2/Data-lotkav/train-lotkav.csv")
df_train = df_train.drop(columns=['Unnamed: 0'],axis=1)
df_test = pd.read_csv("C:/Users/laach/Documents/Skole/Masteroppgave/Master2/Code2/Data-lotkav/test-lotkav.csv")
df_test = df_test.drop(columns=['Unnamed: 0'],axis=1)

x_train = df_train.iloc[:,0:15]
y_train = df_train.iloc[:,15:17]
x_test = df_test.iloc[:,0:15]
y_test = df_test.iloc[:,15:17]
...
x_train2_pre = x_train[['4','9']]
x_train2 = []

for i2 in range(0,(len(x_train2_pre))):
    if i2 != (len(x_train2_pre)-1) :
        dt = (x_train.iloc[i2+1,14]-x_train.iloc[i2,14])
    else: 
        dt = (x_train.iloc[i2,14]-x_train.iloc[i2-1,14])
...
for normal_nn in range(0,nn_number): 
    ...
    for i in range(0,len(x_test-2)):
        ...
        dt = x_test.iloc[1,14] - x_test.iloc[1,13]
...
for input_layer in range(0,4):
    ...
    for guided_nn in range(0,nn_number):
        ...
        output2 = Dense(2,activation='linear')(x2)
...
#normal_nn_results[3] = normal_nn_results[2]
#guided_nn_results[3][14] = guided_nn_results[3][13]
...
line_plot_nn_x.savefig('nn_x-lotkav-noise.pdf')
line_plot_nn_y.savefig('nn_y-lotkav-noise.pdf')
line_plot_l1_x.savefig('l1_x-lotkav-noise.pdf')
line_plot_l1_y.savefig('l1_y-lotkav-noise.pdf')
